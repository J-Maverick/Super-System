...
namespace ArchiTech
{
    ...
    public class TVManagerV2 : UdonSharpBehaviour
    {
        ...
        public void _InternalUpdate()
        {
            ...
            BaseVRCVideoPlayer vp = jumpToTime == EPSILON ? prevManager.player : activeManager.player;
            if (VRC.SDKBase.Utilities.IsValid(vp)) { } else return; // video player has been unloaded, game is closing
            if (mediaEnded) { } // media has ended, nothing to skip
        ...
        public void _PostDeserialization()
        {
            ...
            if (state != stateSync)
            {
                ...
                switch (state)
                {
                    ...
                    case PLAYING: if (locallyPaused) { } else _Play(); break;
                    // if owner pauses, unset the local pause flag
                    case PAUSED: _Pause(); break;
                    default: break;
                }
            }
        ...
        {
            // general media info
            mediaLength = activeManager.player.GetDuration();
        ...
        public void _RefreshMedia()
        {
            ...
            else if (currentState != STOPPED)
            {
                ...
                if (jumpToTime == EPSILON)
                    reloadCache = prevManager.player.GetTime();
                else reloadCache = activeManager.player.GetTime();
        ...
        public void _TogglePlay()
        {
            ...
            else if (currentState == PAUSED) play();
