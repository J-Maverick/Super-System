Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/61c1379190d356441ab747ca8b9db780"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1) == 1)
				{
					...
					#endif
					if (float(0.408) == float(0)) float(0.408) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0.408), float(0), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.408), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0.408), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(1) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0), float(0.408), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1436938,0.1436938,0.1436938,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.1436938,0.1436938,0.1436938,1).a;
				//ifex float(1)==0
				...
				poiFragData.baseColor = saturate(poiFragData.baseColor + float(0.011) * hueShiftAlpha.g);
				#endif
